...
description: Agent skills for working with Minions Stakeholders MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-stakeholders data.
---

# Minions Stakeholders Agent Skills

Skills for agents operating on the `minions-stakeholders` toolbox.
...
```bash
# TypeScript
pnpm add @minions-stakeholders/sdk
...
The `stakeholders` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-stakeholders/sdk/schemas';
...
```python
from minions_stakeholders.schemas import custom_types
...
- This agent only writes to `minions-stakeholders` — it reads from other toolboxes but never writes to them
